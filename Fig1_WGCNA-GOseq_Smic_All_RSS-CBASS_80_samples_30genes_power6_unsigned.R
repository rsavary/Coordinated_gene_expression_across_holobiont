##### WGCNA for all Smicro samples and GOseq enrichment of the modules
...
ddsTxiSKtemp<-counts(ddsTxiSKtemp)[1:49109,] #keep only Smic
design2<-design
...
dim(new2) ##### number of gene retained for WGCNA for Smic: 29557
...
vstCBASScoral3<-vstCBASScoralT1[apply(vstCBASScoralT1,1,var)>0,] # filtered for low variance, still 29557 genes
...
abline(h = 450, col = "red"); # remove 7 outliers
...
Smicro_all_vst_vstCBASScoral6<-vstCBASScoral6
save(Smicro_all_vst_vstCBASScoral6,file = "/Users/romainsavary/Desktop/EPFL-Post-Doc/00_0_Paper_writting/03_Paper_WGCNA_Traits_Smicro/11_WGCNA_last_analysis_with_script_forGithub/Smicro_all_vst_vstCBASScoral6.RData")


####2a automatic network construction and module detection for Smic all (73 samples)
...
##### use table from FAQ of WGCNA and use 6 as the power does not reach exactly 0.8 except at 18 which is too high and should be small than 15
...
SpistCBASS = blockwiseModules(vstCBASScoral6, power = 6, ##### we keep 6 instead of 18 (above 0.8) as suggested for more than 40 samples in the FAQ : https://horvath.genetics.ucla.edu/html/CoexpressionNetwork/Rpackages/WGCNA/faq.html
                              TOMType = "unsigned", minModuleSize =30,
                              reassignThreshold = 0, mergeCutHeight = 0.25,
                              numericLabels = TRUE, pamRespectsDendro = FALSE,
                              saveTOMs = TRUE,
                              saveTOMFileBase = "Smicro ALL",
                              verbose = 3)
...
table(SpistCBASS$colors) # 17 modules from 92 to 14698 genes
...
# change [X] for other block
plotDendroAndColors(SpistCBASS$dendrograms[[4]], mergedColorsSpistCBASS[SpistCBASS$blockGenes[[4]]],
                    "Module colors", main = "Gene dendrogram and module colors in block 1",
                    dendroLabels = FALSE, hang = 0.03,
                    addGuide = TRUE, guideHang = 0.05)
...
save(MEsSpist, moduleLabelsSpist, moduleColorsSpist, geneTreeSpist,
     file = "Smicro_ALL-30genes_p6_unsigned_networkConstruction-auto.RData")
View(table(moduleColorsSpist))
dim(table(moduleColorsSpist))


write.csv(MEsSpist,"MEs_Smicro_ALL_30genes_6.csv")

######### cluster modules 
# Calculate eigengenes
MEsT1 = moduleEigengenes(vstCBASScoral6, moduleColorsSpist)$eigengenes
MEsMT1 = orderMEs(MEsT1)
colnames(MEsMT1)[1]<-paste(colnames(MEsMT1)[1],"Nitrate assimilation", sep=" ")
colnames(MEsMT1)[10]<-paste(colnames(MEsMT1)[10],"Photosynthesis 2", sep=" ")
colnames(MEsMT1)[11]<-paste(colnames(MEsMT1)[11],"Photosynthesis 1", sep=" ")
# Calculate dissimilarity of module eigengenes
MEDiss = 1-cor(MEsMT1);
# Cluster module eigengenes
METree = hclust(as.dist(MEDiss), method = "average");
# Plot the result
pdf(paste("Smicro_module_dendogram_clustering",".pdf",sep="")) 
#sizeGrWindow(7, 6)
...
#############
##### load phenotypic data from Evensen et al., 2021: test one with PAM data make mean data for every samples. after that make a single data frame with the 85 samples and the phenoptypic value associated.
...
pdf(paste("Module-physio_Smicro_All_p18__30genes_10modules_heatmap1",".pdf",sep="")) 
par(mar = c(12, 12, 3, 3))
labeledHeatmap(Matrix = t(geneModuleMembershipM),
               ...
               cex.text = 0.5,
               zlim = c(-1,1),
               main = paste("Module-trait relationships, Smicro- All p6,30genes, 10modules vs Phenotypic traits"))

dev.off()

dev.off()
library("pheatmap")
pdf(paste("Module-physio_Smicro_ALL_p6_30genes_17modules_heatmap2",".pdf",sep="")) 
#par(mar = c(12, 12, 3, 3))
...
b<-pheatmap(t(geneModuleMembershipM),
            ...
            main= paste("WGCNA Smicro-ALL p6, 30genes, 17modules vs traits")
)
dev.off()
...
for(i in 1:length(colnames(MEsMT1))){
  ...
  write.csv(modulemodule7, file = paste0("/Users/romainsavary/Desktop/EPFL-Post-Doc/00_0_Paper_writting/03_Paper_WGCNA_Traits_Smicro/11_WGCNA_last_analysis_with_script_forGithub/Smicro_All",a,".csv"))
  ...
  write.csv(moduleHUB, file = paste0("/Users/romainsavary/Desktop/EPFL-Post-Doc/00_0_Paper_writting/03_Paper_WGCNA_Traits_Smicro/11_WGCNA_last_analysis_with_script_forGithub/Smicro_ALL_HUB_",a,".csv"))
}
...
########### for list of gene from the Smic modules 
setwd("/Users/romainsavary/Desktop/EPFL-Post-Doc/00_0_Paper_writting/03_Paper_WGCNA_Traits_Smicro/08_Last_analysis_modules/02_WGCNA_Smicro_all_80/")

# load file with pattern Spist

filesT1 <- list.files(pattern =  "\\Smicro_")
...
for(i in 1:length(list.data_t1)){
  ...
  write.csv2(newfile4,paste("/Users/romainsavary/Desktop/EPFL-Post-Doc/00_0_Paper_writting/03_Paper_WGCNA_Traits_Smicro/11_WGCNA_last_analysis_with_script_forGithub/",name,"GO_enrichement_",dimen,"_terms.csv"))
}
